...
public class AnimationSlider extends AbstractSliderButton implements ISpecialWidget {
    ...
    public AnimationSlider(int x, int y, Component component, double currentValue, AnimatableEntity<?> animatableEntity, String controllerName, double stepSize, double minValue, double maxValue) {
        super(x, y, 115, 15, component, (currentValue - minValue) / (maxValue - minValue));
    ...
    public void applyValue() {
        try {
            String str = this.controllerName + "=" + getSliderValue();
    ...
    public String getValueString() {
        return DECIMAL_FORMAT.format(getSliderValue());
    }
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.blitWithBorder(ROULETTE_TEXTURE, getX() + ((int) (this.value * (this.width - 8))), getY(), 0, 24, 8, this.height, 200, 15, 2, 3, 2, 2);
