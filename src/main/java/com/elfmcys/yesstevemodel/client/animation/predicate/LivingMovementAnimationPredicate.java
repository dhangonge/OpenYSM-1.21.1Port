...
public class LivingMovementAnimationPredicate implements IAnimationPredicate<LivingAnimatable<?>> {
    ...
    public PlayState renderRidingAnimation(AnimationEvent<LivingAnimatable<?>> event) {
        ...
        // 上游用 LivingEntity（女仆安全）；移植版曾误改成 (Player) 强转，女仆实体（EntityMaid）渲染时必然 ClassCastException
        ...
        boolean isCarrying = (livingEntity instanceof Player) && CarryOnCompat.isPlayerCarrying((Player) livingEntity);
