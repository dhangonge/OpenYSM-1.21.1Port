...
public class EquippedEnchantmentLevel extends LivingEntityFunction {
    ...
    public Object eval(ExecutionContext<IContext<LivingEntity>> context, ArgumentCollection arguments) {
        ...
        var enchantmentRegistry = context.entity().entity().level().registryAccess().registryOrThrow(Registries.ENCHANTMENT);
        int enchantmentLevel = 0;
        for (int i = 1; i < arguments.size(); i++) {
            ResourceLocation id = arguments.getResourceLocation(context, 1);
            if (id != null) {
                enchantmentLevel += enchantmentRegistry.getHolder(ResourceKey.create(Registries.ENCHANTMENT, id)).map(stack::getEnchantmentLevel).orElse(0);
            }
        }
